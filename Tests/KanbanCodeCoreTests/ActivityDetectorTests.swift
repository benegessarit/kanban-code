...
struct ActivityDetectorTests {
    ...
    func interruptFallback() async {
        ...
        let dir = NSTemporaryDirectory() + "kanban-code-interrupt-test-\(UUID().uuidString)"
    ...
    func activeWithinGracePeriod() async {
        ...
        let dir = NSTemporaryDirectory() + "kanban-code-grace-test-\(UUID().uuidString)"
        try? FileManager.default.createDirectory(atPath: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(atPath: dir) }
        let path = (dir as NSString).appendingPathComponent("test.jsonl")
        try? "data".write(toFile: path, atomically: true, encoding: .utf8)
    ...
    func pollRecent() async {
        let dir = NSTemporaryDirectory() + "kanban-code-activity-test-\(UUID().uuidString)"
        try? FileManager.default.createDirectory(atPath: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(atPath: dir) }
